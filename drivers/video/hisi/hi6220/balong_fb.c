...
#if ADE_SYNC_SUPPORT
STATIC int balongfb_fence_inc_thread(void *data)
{
    unsigned long  flag;
    int ret;
    struct balong_fb_data_type *balongfd = (struct balong_fb_data_type *)data;
    ...
    while (!kthread_should_stop()) {
        ret = wait_event_interruptible(balongfd->fence_inc_wait, (balongfd->refresh > 0));
        if (ret == 0) {
            spin_lock_irqsave(&balongfd->refresh_lock, flag);
            sw_sync_timeline_inc(balongfd->timeline, 1);
            balongfd->refresh--;
            spin_unlock_irqrestore(&balongfd->refresh_lock, flag);
        }
    }
    return 0;
}
...
#if ADE_SYNC_SUPPORT
        if (balongfd->refresh && balongfd->fence_inc_thread) {
            wake_up_interruptible(&balongfd->fence_inc_wait);
        }
#endif
...
#if ADE_SYNC_SUPPORT
        if ((balongfd->refresh > 0) && balongfd->fence_inc_thread) {
            wake_up_interruptible(&balongfd->fence_inc_wait);
        }
#endif
        balongfd->update_frame = 1;
...
#if ADE_SYNC_SUPPORT
    init_waitqueue_head(&balongfd->fence_inc_wait);

    balongfd->fence_inc_thread = kthread_run(balongfb_fence_inc_thread, balongfd, "balongfd-fenceinc");
    if (IS_ERR(balongfd->fence_inc_thread)) {
        balongfb_loge("failed to run fenceinc thread\n");
        balongfd->fence_inc_thread = NULL;
    }
#endif
...
STATIC int balong_fb_remove(struct platform_device *pdev)
{
    ...
    if (balongfd->timeline) {
        /* TODO: use the function when porting to Android 4.4 */
        //sw_sync_timeline_destroy(balongfd->timeline);
        balongfd->timeline = NULL;
    }

    if (balongfd->fence_inc_thread) {
        kthread_stop(balongfd->fence_inc_thread);
    }
#endif
